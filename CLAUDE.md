...
이 repo는 **Type A** (npm 패키지 + Changesets 풀스택)다. 현재 **`0.1.x` patch only** 단계, minor 진입 없이 다음 minor 이벤트는 곧바로 **`1.0.0`**. Claude는 changeset에서 patch만 자율 생성, minor/major는 Dave 명시 지시 시만.
