...
기본은 **명시적 `install()`** 이다 — tree-shaking을 위해 `package.json` `sideEffects`는 부수효과가 실제로 있는 entry(`auto`·`sentinel`)만 나열하고, 나머지 import는 호출자가 installer를 직접 부르기 전엔 아무것도 안 한다. detect만 쓰고 싶을 때 clipboard shim까지 끌려오는 것을 방지.
