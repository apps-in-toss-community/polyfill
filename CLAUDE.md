...
- **`localStorage` ↔ SDK Storage** — **out-of-scope**. SDK에 Storage 함수 자체가 없다 (web-bridge dist에 storage 관련 export 0개). 표준 `localStorage`는 토스 WebView에서 native로 동작하므로 polyfill 자체가 불필요. 가상의 RN-bridge async storage가 있더라도 sync `localStorage` 시맨틱과 화해 불가 (소비자가 sync 가정).
...
- **Tier 2에서 격상된 항목**: `localStorage` (SDK 미제공 + sync/async mismatch), `history.back` (`closeView`와 시맨틱 mismatch — 미니앱 종료 ≠ nav pop), `visibilitychange` (표준이 토스 WebView에서 이미 동작).
